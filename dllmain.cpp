...
//    if (!logFile) fopen_s(&logFile, "jc3_rebel_orient.log", "w");  // 'w' = fresh each run
//    if (logFile) {
//        va_list args; va_start(args, format);
//        vfprintf(logFile, format, args); va_end(args);
//        fflush(logFile);
//    }
//}
//
...
//}
//
//// Count unit-length (0.9–1.1) vectors in [base, base+0x400). The real camera
//// manager is "rich" (dozens); a transient garbage block holding the vtable
//// word is sparse.
...
//
//// ============================================================================
//// CAMERA MANAGER DISCOVERY
//// ============================================================================
//static int FindAllCameraManagers(uintptr_t* out, int maxOut) {
//    int count = 0;
...
//                        if (buf[i] == targetVTable) {
//                            if (count < maxOut) out[count] = reinterpret_cast<uintptr_t>(&buf[i]);
//                            count++;
//                        }
//                }
...
//}
//
//// Cheap per-spawn validation of a cached pointer: vtable still present AND
//// +0x044 still holds a unit vector (the live forward). Both must hold.
...
//}
//
//// Select the real camera manager: richest candidate whose +0x044 is a live
//// forward. Cached; re-selected only when the cache is invalidated.
//uintptr_t GetLiveCameraManager() {
//    static uintptr_t cached = 0;
//    if (IsLiveCameraManager(cached)) return cached;
//
//    uintptr_t cands[8];
//    const int n = FindAllCameraManagers(cands, 8);
//
//    uintptr_t best = 0;
//    int bestScore = -1;
//    for (int i = 0; i < n; i++) {
//        float v[3] = { 0, 0, 0 };
//        if (!SafeReadMemory(cands[i] + OFFSET_CAMERA_FORWARD, v, sizeof(v))) continue;
//        const float len = VecLen(v);
//        if (len < 0.9f || len > 1.1f) continue;        // must hold a live forward at 0x044
//        const int score = CountUnitVectors(cands[i]);
//        if (score > bestScore) { bestScore = score; best = cands[i]; }
//    }
//
//    cached = best;
//    if (best)
//        LogMessage("[Cam] selected live manager 0x%llX (richness %d, %d candidates)\n", best, bestScore, n);
//    else
//        LogMessage("[Cam] WARNING: no valid live manager found (%d candidates)\n", n);
//    return best;
//}
//
//bool GetCameraForwardVector(float& outX, float& outZ) {
//    const uintptr_t mgr = GetLiveCameraManager();
//    if (mgr == 0) return false;
//
//    float fX, fY, fZ;
...
//
//                LogMessage("[Spawner] aligned to camera, yaw=%.1f deg\n",
//                    atan2f(fX, fZ) * (180.0f / 3.14159265f));
//            }
//            else {
//                LogMessage("[Spawner] fallback: no live camera forward\n");
//            }
//        }
//        __except (EXCEPTION_EXECUTE_HANDLER) {
